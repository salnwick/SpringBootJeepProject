...
public class DefaultJeepOrderDao implements JeepOrderDao {
    ...
    public Order saveOrder( Customer customer, Jeep jeep, Color color, Engine engine, Tire tire, BigDecimal price, List<Option> options ) {
        ...
        // Strategy is to first write the order without the options, and then use its pk to form the options
        //   via the many-to-many relationship inherent in the order_options table      
        SqlParams params = generateInsertSql( customer, jeep, color, engine, tire, price );
        KeyHolder keyHolder = new GeneratedKeyHolder();  // receives the primary key when we call update
    ...
    public List<Option> fetchOptions( List<String> optionIds ) {
        ...
        for ( int index = 0; index < optionIds.size(); index++ ) { // :option_0, :option_1, ...
    ...
    public Optional<Customer> fetchCustomer( String customerId ) {
        ...
        return Optional.ofNullable( jdbcTemplate.query( sql, params, new CustomerResultSetExtractor() ) );  // empty, or the value
    }
    ...
    class CustomerResultSetExtractor implements ResultSetExtractor<Customer> {
        ...
        public Customer extractData( ResultSet rs ) throws SQLException, DataAccessException {
            rs.next();  // starts pre-initialized, so get to the first row
    ...
    class SqlParams {
        String sql;
        MapSqlParameterSource source = new MapSqlParameterSource();  // hashmap but with an addValue() method instead of put()
    }
